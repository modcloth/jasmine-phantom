...
var totalSpecTimeoutSeconds = 60 * 10;
...
function waitForSpecsToFinish(page, seconds, whenDone) {
  if (jasmineReportsFinished(page)) {
    ...
    if (seconds > 0) {
      ...
      console.log("Jasmine tests didn't finish in " + totalSpecTimeoutSeconds + " seconds... bailing out");
      phantom.exit(2);
    }
  }
}
...
page.open(system.args[1], function(status) {
  if (status == 'success') {
    waitForSpecsToFinish(page, totalSpecTimeoutSeconds, function() {
